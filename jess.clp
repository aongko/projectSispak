...
(defrule cari-sriwijaya
        ?i <- (cari-sriwijaya (num ?num))
        ?j <- (sriwijaya ?id ?class ?date ?origin ?destination ?price ?seat)
        =>
        (if (= ?*counter* ?num) then
                (retract ?i)
                (retract ?j)
...
(defrule ticket-simulation
    ...
    (if (eq (str-compare ?typeOfFlight "Domestic") 0) then
        (?v add "Citilink")
        (?v add "Indonesia Air Asia")
        (?v add "Lion Air")
        (?v add "Mandala Airlines")
    else
        (?v add "Garuda Indonesia")
        (?v add "Sriwijaya Air")
    )
    (new Main (context) ?typeOfFlight ?budget ?origin ?destination ?dateOfDeparture ?numberOfTickets);
    (assert(mainmenu))
)

(defquery query-citilink
    (declare (variables ?qbudget ?qorigin ?qdestination ?qdate))
    (citilink ?id ?class ?date ?origin ?destination ?price ?seat)
    (test (< ?price ?qbudget))
    (test (eq (str-compare ?qorigin ?origin) 0))
    (test (eq (str-compare ?destination ?qdestination) 0))
    (test (eq (str-compare ?date ?qdate) 0))
)

(defquery query-indonesia-air-asia
    (declare (variables ?qbudget ?qorigin ?qdestination ?qdate))
    (airasia ?id ?class ?date ?origin ?destination ?price ?seat)
    (test (< ?price ?qbudget))
    (test (eq (str-compare ?qorigin ?origin) 0))
...
(defquery query-mandala-airlines
    (declare (variables ?qbudget ?qorigin ?qdestination ?qdate))
    (mandala ?id ?class ?date ?origin ?destination ?price ?seat)
    (test (< ?price ?qbudget))
    (test (eq (str-compare ?qorigin ?origin) 0))
...
(defquery query-garuda-indonesia
    (declare (variables ?qbudget ?qorigin ?qdestination ?qdate))
    (garuda ?id ?class ?date ?origin ?destination ?price ?seat)
    (test (< ?price ?qbudget))
    (test (eq (str-compare ?qorigin ?origin) 0))
